  ...
  function renderBaseline() {
    return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ background: T.card, borderRadius: 10, border: "1px solid " + T.border, padding: "16px 18px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "130px 56px 100px 90px 20px", gap: 8, paddingBottom: 6, borderBottom: "2px solid " + T.border, marginBottom: 4 }}>
          <span style={{ fontFamily: T.m, fontSize: 8, color: T.td, textTransform: "uppercase", letterSpacing: 0.8 }}>Type</span>
          <span style={{ fontFamily: T.m, fontSize: 8, color: T.td, textTransform: "uppercase", letterSpacing: 0.8, textAlign: "center" }}>Count</span>
        ...
        {sites.map(function (s, i) {
          return (<div key={s.id} style={{ padding: "8px 0", borderBottom: i < sites.length - 1 ? "1px solid " + T.border : "none" }}>
            <div style={{ display: "grid", gridTemplateColumns: "130px 56px 100px 90px 20px", gap: 8, alignItems: "center" }}>
              <select value={s.type} onChange={function (e) { setSites(sites.map(function (x) { return x.id === s.id ? Object.assign({}, x, { type: e.target.value }) : x; })); }} style={Object.assign({}, selS, { fontSize: 10, fontWeight: 600, width: "100%" })}>{["Branch", "Acquired", "HQ / Campus", "Data Center", "DR Site", "Remote", "Retail", "Warehouse", "Contact Center"].map(function (o) { return <option key={o}>{o}</option>; })}</select>
              <input type="number" value={s.count} onChange={function (e) { setSites(sites.map(function (x) { return x.id === s.id ? Object.assign({}, x, { count: Number(e.target.value) || 0 }) : x; })); }} style={Object.assign({}, smI, { width: "100%", textAlign: "center", fontSize: 12, fontWeight: 600, padding: "5px 6px" })} />
              <select value={s.circuit} onChange={function (e) { setSites(sites.map(function (x) { return x.id === s.id ? Object.assign({}, x, { circuit: e.target.value }) : x; })); }} style={Object.assign({}, selS, { fontSize: 10, width: "100%" })}>{["MPLS", "DIA", "Broadband", "LTE/5G", "MPLS+DIA", "BB+LTE", "Metro Eth", "Fiber P2P", "None"].map(function (o) { return <option key={o}>{o}</option>; })}</select>
              <select value={s.bandwidth} onChange={function (e) { setSites(sites.map(function (x) { return x.id === s.id ? Object.assign({}, x, { bandwidth: e.target.value }) : x; })); }} style={Object.assign({}, selS, { fontSize: 10, width: "100%" })}>{["10 Mbps", "25 Mbps", "50 Mbps", "100 Mbps", "200 Mbps", "500 Mbps", "1 Gbps", "10 Gbps"].map(function (o) { return <option key={o}>{o}</option>; })}</select>
